...
const OrganizationSubscriptionPurchase = () => {
  ...
  useEffect(() => {
    ...
    const saved = sessionStorage.getItem('pendingOrgPayment');
    if (saved) {
      try {
        ...
        sessionStorage.removeItem('pendingOrgPayment');
      }
    }
  }, []);
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (success || status === 'completed') {
          ...
          // Refresh identity so the dashboard's subscription gate unblocks immediately
          try {
          ...
          setTimeout(() => navigate('/org-admin'), 3000);
  ...
  const fetchPlans = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await api.get('/organization-plans/active');
  ...
  const handlePurchase = async () => {
    ...
    try {
      ...
      const response = await api.post('/subscriptions/organization/initiate', payload);
      ...
      if (response.data.success) {
        if (response.data.paymentUrl) {
          ...
          sessionStorage.setItem('pendingOrgPayment', JSON.stringify({
      ...
      console.error('Error purchasing organization subscription:', err);
      ...
      <Container maxWidth="sm" sx={{ mt: 8, textAlign: 'center' }}>
        <Paper elevation={3} sx={{ p: 6, borderRadius: 3 }}>
          ...
          <Typography variant="body1" color="text.secondary" sx={{ mb: 1 }}>
            Your organisation subscription is now active.
          </Typography>
    ...
    <Container maxWidth="lg" sx={{ mt: 4, mb: 8 }}>
      ...
      <Box sx={{ textAlign: 'center', mb: 4 }}>
        <Typography variant="h4" fontWeight="bold" gutterBottom sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 2 }}>
          ...
          Choose Your Organisation Plan
        </Typography>
        <Typography variant="body1" color="text.secondary">
          Select a plan for {user?.organization || 'your organisation'} to unlock more teachers, students and exams
        </Typography>
      </Box>
      ...
      {!loading && plans.length === 0 && !mobilePending ? (
        <Paper sx={{ p: 4, textAlign: 'center', borderRadius: 2 }}>
          <Typography variant="h6" color="text.secondary">
            No organisation plans available yet.
          </Typography>
          <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
...
export default OrganizationSubscriptionPurchase;
