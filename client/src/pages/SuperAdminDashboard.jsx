...
const nav = [
  ...
  { id: 'individual-plans', label: 'Individual Plans', icon: <Person sx={{ fontSize: 20 }} /> },
...
export default function SuperAdminDashboard() {
  ...
  return (
    <DashboardShell
      ...
      {activeSection === 'individual-plans' && <IndividualPlanManagement />}
          ...
          <Paper elevation={0} sx={{ p:2.5, borderRadius:3, border:`1px solid ${tokens.surfaceBorder}`, bgcolor:'white' }}>
            ...
            {loadingOrgs?<CircularProgress size={22} sx={{color:tokens.accent}}/>:orgs.length===0?<Typography sx={{color:tokens.textMuted,fontSize:13}}>No organizations yet.</Typography>:(
              <TableContainer><Table size="small">
                <TableHead><TableRow sx={{bgcolor:'#F8FAFC'}}>{['Organization','Sign-in','Plan','Status'].map(h=><TableCell key={h} sx={{fontWeight:700,color:tokens.textSecondary,fontSize:12}}>{h}</TableCell>)}</TableRow></TableHead>
                <TableBody>
                  {orgs.map(o=>(
                    <TableRow key={o._id} sx={{'&:hover':{bgcolor:'#F8FAFC'}}}>
                      ...
                      <TableCell>
                        ...
                        ) : o.signinMethod === 'phone' ? (
                          <Tooltip title="Phone Number Sign-in">
                            <Box sx={{display:'flex',alignItems:'center',gap:0.5}}>
                              <Phone fontSize="small" sx={{color:tokens.accent,fontSize:14}}/>
                            </Box>
                          </Tooltip>
                        ) : (
                          <Tooltip title="Email/Password Sign-in">
                            <Email fontSize="small" sx={{color:tokens.textSecondary,fontSize:14}}/>
                          </Tooltip>
                        )}
                      </TableCell>
                  ...
                  <Box sx={{display:'flex',alignItems:'center',justifyContent:'space-between',pt:1.5,borderTop:`1px solid ${tokens.surfaceBorder}`}}>
                    <Box sx={{display:'flex',alignItems:'center',gap:1}}>
                      {o.signinMethod === 'google' ? (
                        ...
                        <Tooltip title="Phone Number Sign-in">
                          <Phone fontSize="small" sx={{color:tokens.accent,fontSize:12}}/>
                        </Tooltip>
                      ) : (
                        <Tooltip title="Email/Password Sign-in">
      ...
      {loading?<Box sx={{display:'flex',justifyContent:'center',mt:6}}><CircularProgress sx={{color:tokens.accent}}/></Box>:(
        <Paper elevation={0} sx={{borderRadius:3,border:`1px solid ${tokens.surfaceBorder}`,bgcolor:'white',overflow:'hidden'}}>
          <TableContainer><Table>
            <TableHead><TableRow sx={{bgcolor:'#F8FAFC'}}>{['User','Role','Type/Org','Sign-in Method','Plan','Status','Actions'].map(h=><TableCell key={h} sx={{fontWeight:700,color:tokens.textSecondary,fontSize:12}}>{h}</TableCell>)}</TableRow></TableHead>
            <TableBody>
              {filtered.length===0?<TableRow><TableCell colSpan={7} align="center" sx={{py:5,color:tokens.textMuted}}>No users found.</TableCell></TableRow>:
              filtered.map(u=>{
                      ...
                      {u.signinMethod === 'google' ? (
                        <Tooltip title="Google Sign-in">
                          <Box sx={{display:'flex',alignItems:'center',gap:0.5}}>
                            ...
                            <Typography variant="caption" sx={{fontWeight:600,color:tokens.textSecondary}}>Google</Typography>
                          </Box>
                        </Tooltip>
                      ) : u.signinMethod === 'phone' ? (
                        <Tooltip title="Phone Number Sign-in">
                          <Box sx={{display:'flex',alignItems:'center',gap:0.5}}>
                            <Phone fontSize="small" sx={{color:tokens.accent,fontSize:16}}/>
                            <Typography variant="caption" sx={{fontWeight:600,color:tokens.accent}}>Phone</Typography>
                          </Box>
                        </Tooltip>
                      ) : (
                        <Tooltip title="Email/Password Sign-in">
                          <Box sx={{display:'flex',alignItems:'center',gap:0.5}}>
                            <Email fontSize="small" sx={{color:tokens.textSecondary,fontSize:16}}/>
                            <Typography variant="caption" sx={{fontWeight:600,color:tokens.textSecondary}}>Email</Typography>
                          </Box>
                        </Tooltip>
                      )}
                    </Box>
