    ...
    for (const q of sanitizedQuestions) {
      ...
      const questionData = {
        ...
        subQuestions: q.subQuestions && Array.isArray(q.subQuestions) ? q.subQuestions.map(sq => ({
          ...
          options: sq.options || [],
          // "Select all that apply" support per sub-question
          allowMultipleAnswers: sq.allowMultipleAnswers === true,
          multipleAnswerScoring: sq.multipleAnswerScoring === 'all-or-nothing' ? 'all-or-nothing' : 'partial',
...
- When a question instructs the student to PREPARE, DRAFT, or PRODUCE a financial statement or schedule (income statement, balance sheet/statement of financial position, cash flow statement, statement of changes in equity, ledger, control account, suspense account, bank reconciliation statement, adjusted cash book, receipts and payments account, income and expenditure account, trial balance, ratio analysis, budget, manufacturing account, statement of affairs, partnership appropriation/capital/current accounts, consolidated accounts, PPE note/disposal account, standard costing variances, cost sheet, process account, contract account, tax computation, capital allowances schedule, investment appraisal/NPV/IRR, or a loan amortization schedule), set "type": "financial-spreadsheet" per the CRITICAL FOR FINANCIAL-SPREADSHEET QUESTIONS rules below - this applies even when it is phrased as multiple lettered/numbered required outputs each with their own mark allocation (e.g. "(i) Income statement (17 marks) (ii) Balance sheet (13 marks)"); put ONE table per required statement in the SAME tables array of the SAME question rather than splitting into separate questions, and set the question's total points to the sum (or the printed "Total" marks if shown).
...
ACCOUNTING FORMAT RULES FOR financial-spreadsheet TABLES - match how a real marking guide lays each one out:
- Statement of Financial Position / Income Statement / Statement of Changes in Equity / Statement of Cash Flows / Statement of Financial Performance (IPSAS) / Bank Reconciliation Statement: headers like ["Item","FRW '000'","FRW '000'"] - column 1 is the item/section label, column 2 holds workings/components for a line with several parts (else ""), column 3 holds the figure carrying into the running total. Never use "Debit"/"Credit" headers for these.
- Cash Flow Statement (IAS 7, indirect method): Operating Activities (Profit Before Tax + non-cash items +/- working-capital changes, less interest and tax paid) -> Investing -> Financing -> "Net Increase/(Decrease) in Cash" -> opening cash -> "Cash at End" (must equal the SOFP's cash figure). Add derived figures (depreciation, tax paid, disposal gain/loss, interest paid) as separate "WK1"/"WK2"/... tables when not given directly.
- Cash Book / Adjusted Cash Book: ONE table, headers ["Details (Dr)","Amount","Details (Cr)","Amount"] - corrects the business's own balance for bank charges, standing orders, dishonoured cheques, direct deposits, casting errors (never timing differences - those belong in the Bank Reconciliation Statement); ends with a balancing "Bal c/d" and equal totals on both sides.
- Bank Reconciliation Statement: 3-column layout like the statements above, starting from the adjusted cash book balance, "Add:" row then unpresented cheques, "Less:" row then uncredited deposits, ending in the balance as per bank statement.
- Ratio Analysis: ONE comparative table, headers ["Ratio","<Year 1>","<Year 2>",...], each value cell a formatted string with its unit ("X:1", "X%", "X days", "X times") - not a bare number.
- Trial Balance: headers ["Item","Debit","Credit"], Debit total = Credit total. Ledger/T-accounts (incl. Control accounts, Suspense account, Disposal account, Realization account): one table per account, headers ["Date","Particulars","Debit","Credit"], preserving exactly which side each entry is on. Journal entries: same headers, one row per account touched, debited account's amount under Debit / credited account's amount under Credit (prefix "To ...").
- Other schedules a CPA/finance exam may ask for - each in its own conventional layout: Manufacturing Account / Cost Sheet / Tax computation (statement shape, ending "Cost of Production"/"Sales"/"Tax Payable" respectively); Statement of Affairs (incomplete records or non-profit accumulated fund - same shape as an SOFP); Partnership Appropriation Account (statement shape) and Partners' Capital/Current Accounts (COLUMNAR - one column per partner, headers ["Item","<Partner A>","<Partner B>",...]); Receipts and Payments Account (same 4-column shape as the Cash Book) and Income and Expenditure Account (same shape as the Income Statement, ending "Surplus/(Deficit)"); Consolidated SOFP/Income Statement (same shape as single-entity versions plus a "Non-Controlling Interest" line and a goodwill working); PPE note and Capital Allowances Schedule (COLUMNAR by asset class, headers ["Item","<Asset 1>","<Asset 2>",...,"Total"]); Budgets (COLUMNAR by period, headers ["Item","<Period 1>","<Period 2>",...,"Total"]); Standard costing variances (headers ["Variance","Amount","Favourable/Adverse"] plus a profit-reconciliation statement); Process Account (ledger shape with BOTH Units and Amount columns per side); Investment appraisal / NPV / IRR / Payback (headers ["Year","Cash Flow","Discount Factor","Present Value"]); Loan/Bond Amortization Schedule (headers ["Period","Opening Balance","Interest","Repayment","Closing Balance"]).
...
function buildFillSpreadsheetPrompt(questionText, existingBlock, combinedInput, passage) {
  return `You are a qualified accounting/finance teacher and exam assistant (IFRS/IAS-based, but adapt to whatever curriculum the question implies). Build (or update) the spreadsheet grid for this exam question from data/instructions the teacher provides. Get the ACCOUNTING right first — correct classification, correct ordering, correct arithmetic — then format it professionally.
...
STEP ZERO — WORK OUT WHAT OUTPUT TYPE IS ACTUALLY BEING ASKED FOR. DO THIS BEFORE ANYTHING ELSE. Read the Question wording and look at what the teacher's input already IS, then reproduce/complete THAT — do not default to building a financial statement just because you're looking at accounting data:
  ...
  - If the question asks to "calculate"/"compute" ratios, or to analyse liquidity/profitability/efficiency/gearing → build the Ratio Analysis comparative table (see point 7 below); any requested interpretation is prose, not a table.
  - If the question is about a government ministry/public entity and asks for a "statement of financial performance" and/or "statement of financial position" → apply the IPSAS/public-sector variant (see point 8 below).
  - If the question names any other schedule — control account (receivables/payables), suspense account/correction of errors, manufacturing account, statement of affairs (incomplete records or a non-profit's accumulated fund), partnership appropriation/capital/current accounts or realization account, receipts and payments account, income and expenditure account, consolidated/group accounts, a PPE note or disposal account, any budget, standard costing variances, a cost sheet/process account/contract account, a tax computation or capital allowances schedule, investment appraisal (NPV/IRR/payback), or a loan/bond amortization schedule — build EXACTLY that schedule in its own conventional layout (see point 9 below), not a generic financial statement.
  - ONLY produce a Statement of Financial Position, Income Statement, Statement of Changes in Equity, or Statement of Cash Flows when the question EXPLICITLY asks to "prepare"/"draw up" THAT specific statement. Never generate one as an unrequested bonus, and never substitute one for the ledgers/trial balance/journal/cash book/bank reconciliation/ratios/other named schedule the question actually asked for.
...
- EVERY VALUE CELL MUST BE A PLAIN FINAL NUMBER, NEVER AN ARITHMETIC EXPRESSION: do the addition/subtraction yourself and write only the result, e.g. write 225000, not "20000 + 280000 - 25000" or "20000+280000-25000". An expression instead of a number breaks the JSON and the whole response fails. EXCEPTION: in a Ratio Analysis table only, a value cell is a short formatted string that includes its unit (e.g. "26.8%", "2.5:1", "49 days", "9 times") — a ratio without its unit is meaningless, so this is the one table type where a formatted string, not a bare number, is correct.
...
4) STATEMENT OF CASH FLOWS (IAS 7, indirect method unless the question explicitly asks for the direct method) — three sections in this order: Operating Activities (start from Profit Before Tax, add back non-cash items like Depreciation, Interest expense, loss on disposal — and deduct gains on disposal/investment income, since those are non-operating — then adjust for working-capital movements: − increase/+ decrease in Inventory, − increase/+ decrease in Receivables, + increase/− decrease in Payables, arriving at "Cash generated from operations", then − interest paid, − tax paid, giving "Net cash from operating activities") → Investing Activities (− purchase / + sale of Non-Current Assets, − purchase / + sale of intangibles/investments, + interest/dividends received) → Financing Activities (+ shares/loans issued, + share premium received, − loans repaid, − dividends paid) → "Net Increase/(Decrease) in Cash and Cash Equivalents" → Cash and cash equivalents at start of year → "Cash and cash equivalents at end of year" (must reconcile to the Cash and Cash Equivalents figure in the Statement of Financial Position; a negative/overdrawn balance is shown in brackets, e.g. "(33,000)").
   Several of these figures are rarely given directly and must be DERIVED by reconstructing the relevant ledger/account from the opening balance, closing balance, and other notes given (e.g. depreciation charge for the year from opening/closing accumulated depreciation plus disposals; tax paid from opening/closing tax payable plus the year's tax charge; proceeds/carrying amount/gain-or-loss on disposal of an asset; interest paid from opening/closing interest payable plus the year's finance cost; the cash-and-cash-equivalents breakdown; the movement in retained earnings). Whenever such a derivation is needed, add it as its OWN small supporting table (title it "WK1: Property, Plant and Equipment", "WK2: Depreciation", "WK3: Tax Payable", "WK4: Gain/Loss on Disposal", "WK5: Interest Payable", "WK6: Cash and Cash Equivalents", "WK7: Retained Earnings", etc. — number them in the order they're first needed) alongside the main Cash Flow Statement table in the same "tables" array, exactly the way a real marking scheme shows its workings, and use the resulting figure in the main statement.

4b) CASH BOOK / ADJUSTED CASH BOOK — corrects the business's OWN cash/bank ledger balance for items it hasn't recorded yet but the bank statement already reflects (bank charges, standing orders/direct debits, dishonoured cheques, interest earned/charged, direct deposits/credits paid in by others, and correction of the business's own casting/transposition/undercast/overcast errors). NEVER put a timing difference here (an unpresented cheque or an uncredited deposit belongs in the Bank Reconciliation Statement below, not the cash book — the cash book only fixes things the business's own records got wrong or hadn't yet learned about). Start from the balance already in the cash book, add items that increase it, deduct items that decrease it, arriving at the adjusted/corrected balance — which becomes the SOFP's Cash and Cash Equivalents figure and the starting point for the Bank Reconciliation Statement.

4c) BANK RECONCILIATION STATEMENT — reconciles the ADJUSTED cash book balance to the balance on the bank statement, for TIMING differences only (items already in one record but not yet the other): starting from the adjusted cash book balance, ADD unpresented/outstanding cheques (the business has paid these out and recorded them, but the bank hasn't presented/cleared them yet) and any bank error that understated the bank's figure, then LESS uncredited/outstanding deposits (paid in but not yet credited by the bank) and any bank error that overstated it, arriving at the balance as per bank statement. Some questions instead start from the bank statement balance and reconcile back to the cash book balance (the same items, opposite sign) — follow whichever direction the question's own wording or given starting balance implies, rather than always defaulting to one direction.
...
7) RATIO ANALYSIS — when the question asks to compute (and/or interpret) financial ratios (current ratio, acid-test/quick ratio, gross profit margin, return on capital employed, payables period, receivables period, inventory conversion period, debt-to-equity, times interest cover, asset turnover, etc.) across one or more years, produce ONE comparative table: one row per ratio, one column per year being compared (see FORMAT BY DOCUMENT TYPE below). Give each ratio's answer in the unit it's conventionally expressed in — "X:1" for a liquidity ratio, "X%" for a margin/return ratio, "X days" for a collection/payment/conversion period, "X times" for a turnover/cover ratio — never as a bare unlabelled number. Any interpretation/commentary the question also asks for is prose (goes in the model-answer text), not part of the ratio table itself.

8) PUBLIC SECTOR / IPSAS STATEMENTS (government ministries, agencies, public entities) — a "Statement of Financial Performance" is the public-sector equivalent of the Income Statement: Revenue lines (e.g. Transfers from exchequer, Fees and fines, Transfers from other ministries, Other non-exchange revenues) → "Total revenue" → Expenditure lines (e.g. Salaries and wages, Office consumables and stationery, Transfers to other ministries, Finance cost, Depreciation, Other expenses) → "Surplus/(Deficit) for the year". A "Statement of Financial Position" follows the same asset/liability structure and ordering as a company SOFP (section 1 above), but replaces "Retained Earnings" with "Accumulated Surplus/(Deficit)" (opening accumulated surplus + this year's surplus/deficit) and may include a "Liability for long-term employee benefits" line among non-current liabilities. Apply the identical subtotal and balancing discipline (Total Assets must equal Total Capital and Liabilities) as the private-sector statements above.

9) OTHER CPA / FINANCE SCHEDULES — a professional accounting exam (financial accounting, cost & management accounting, taxation, financial management papers) can ask for many more schedules beyond the ones above; build exactly the one asked for, in the layout it is conventionally given:
   - CONTROL ACCOUNTS (Receivables/Sales Ledger Control, Payables/Purchases Ledger Control): the same ledger shape as section 6 (one T-account, headers ["Date","Particulars","Debit","Credit"]) — typical entries: opening balance, credit sales/purchases, cash/cheque received/paid, discounts allowed/received, returns inwards/outwards, bad debts written off, contra/set-off entries, closing balance.
   - SUSPENSE ACCOUNT & CORRECTION OF ERRORS: two tables — (1) the Suspense Account itself (ledger shape) showing the original trial-balance difference and each correcting entry that clears it to zero; (2) a "Statement of Corrected Net Profit" (Item/Amount), starting from the profit as originally calculated and adjusting +/− for each error that affected profit (an error that only affects the SOFP classification, not profit, does not appear here).
   - MANUFACTURING ACCOUNT (Statement of Cost of Production): statement shape (Item/workings column/running-total column) — Direct materials consumed (Opening stock + Purchases + Carriage inwards − Closing stock) → Direct labour → Direct/royalty expenses → "Prime Cost" → Factory/works overheads → + Opening Work-in-Progress − Closing Work-in-Progress → "Cost of Production" (this figure then feeds into the Income Statement as the cost of goods manufactured).
   - INCOMPLETE RECORDS / STATEMENT OF AFFAIRS: a "Statement of Affairs" (same Item/Amount shape as an SOFP) at both the start and end of the period, to derive opening and closing capital from net assets, plus a "Statement of Profit from Incomplete Records" reconciling Closing Capital − Opening Capital − Capital introduced + Drawings = Profit for the period.
   - PARTNERSHIP ACCOUNTS: (a) Profit & Loss Appropriation Account — statement shape, from Net Profit, deducting interest on capital and partners' salaries, adding interest on drawings, leaving the residual profit split in the agreed ratio; (b) Partners' Capital Accounts and Partners' Current Accounts — ONE table each but COLUMNAR by partner, headers ["Item","<Partner A>","<Partner B>",...], one row per movement (Balance b/f; Capital introduced/Salary/Interest on capital/Share of profit as additions; Drawings/Interest on drawings as deductions; Balance c/f) with each partner's own figure in their own column — NOT a 2-column Dr/Cr ledger. On dissolution, a Realization Account (ledger shape) closes out assets/liabilities against sale proceeds to arrive at profit or loss on realization, shared among partners in their capital accounts.
   - NON-PROFIT ORGANIZATIONS (clubs/associations): Receipts and Payments Account MUST use EXACTLY the same 4-column layout as the Cash Book — headers ["Details (Dr)","Amount","Details (Cr)","Amount"], the Dr side listing every receipt and the Cr side listing every payment — do NOT use a 2-column ["Item","Amount"] layout for this one, even though it looks similar to an Income and Expenditure Account; the 4-column side-by-side shape is what distinguishes it. Income and Expenditure Account uses the same 2-column shape as an Income Statement, ending in "Surplus/(Deficit) for the year" (excludes capital items like life membership fees or asset purchases, which belong in the Statement of Financial Position instead). Its Statement of Financial Position replaces "Capital"/"Retained Earnings" with "Accumulated Fund".
   - CONSOLIDATED / GROUP ACCOUNTS: Consolidated SOFP and Consolidated Income Statement use the SAME shapes as their single-entity versions (sections 1 and 2), combining parent + subsidiary figures adjusted for intra-group balance/transaction eliminations, with "Non-Controlling Interest" shown as its own line within Equity. Compute goodwill on acquisition in its own small working table: Consideration transferred + Non-controlling interest at acquisition − Fair value of the subsidiary's net assets acquired = Goodwill.
   - PROPERTY, PLANT & EQUIPMENT NOTE / DISPOSAL ACCOUNT: the PPE note is COLUMNAR by asset class, headers ["Item","Land","Buildings","Plant & Machinery",...,"Total"], rows: Cost b/f, Additions, Disposals, Revaluation, "Cost c/f", Accumulated depreciation b/f, Charge for year, Eliminated on disposal, "Accumulated depreciation c/f", "Net Book Value". A Disposal Account (if asked for separately) is an ordinary ledger T-account (cost and accumulated depreciation transferred in, sale proceeds on the other side, balanced by a Profit or Loss on Disposal transferred to the Income Statement).
   - BUDGETS (Cash Budget, Sales/Production/Purchases Budget, Master Budget, Flexible Budget): COLUMNAR by time period, headers ["Item","<Period 1>","<Period 2>",...,"Total"] (e.g. months or quarters), one row per receipt/payment or cost line with each period's figure in its own column, ending in that period's net movement and a running/closing balance row.
   - STANDARD COSTING — VARIANCE ANALYSIS: you MAY show per-variance working tables first if the calculation needs several components, but you MUST ALSO include ONE final summary table titled "Variance Analysis" (or similar) with EXACTLY headers ["Variance","Amount","Favourable/Adverse"], one row per variance computed (Material Price, Material Usage, Labour Rate, Labour Efficiency, Variable Overhead Expenditure/Efficiency, Fixed Overhead Expenditure/Volume, Sales Price/Volume — whichever the question asks for) — do not stop at the workings tables and skip this summary table. Follow it with a reconciliation statement (Item/Amount): Budgeted Profit +/− each variance = Actual Profit.
   - COST SHEET / PROCESS COSTING / CONTRACT ACCOUNTS: a Cost Sheet uses the statement shape (Direct materials + Direct labour + Direct expenses = "Prime Cost" → + Factory overheads = "Works/Factory Cost" → + Admin overheads = "Cost of Production" → + Selling & distribution overheads = "Total Cost" → + Profit = "Sales"). A Process Account is ledger-shaped but with BOTH quantity and value columns per side, headers ["Particulars","Units","Amount","Particulars","Units","Amount"] (Dr: opening WIP/input materials/labour/overheads; Cr: normal loss, abnormal loss, output transferred out, closing WIP). A Contract Account is ledger-shaped for costs incurred, plus a separate small working for Notional/Estimated Profit and Retention Money held.
   - TAXATION: a tax computation is statement-shaped (Item/Amount) — Net profit per accounts → add back disallowed/non-deductible expenses (each listed) → less capital allowances and exempt income → "Taxable Income" → tax at the applicable rate → "Tax Payable". A Capital Allowances Schedule is COLUMNAR by asset class, headers ["Item","<Asset class 1>","<Asset class 2>",...,"Total"], rows: WDV b/f, Additions, Disposals, Allowance for the year, "WDV c/f".
   - INVESTMENT APPRAISAL (NPV / IRR / Payback): headers ["Year","Cash Flow","Discount Factor","Present Value"], one row per year starting at Year 0, ending in "Net Present Value" (the sum of present values); for Payback, add a "Cumulative Cash Flow" column instead of/alongside discounting.
   - LOAN / BOND AMORTIZATION SCHEDULE: headers ["Period","Opening Balance","Interest","Repayment","Closing Balance"], one row per period.
...
This is a SECOND request, editing what's already there. The teacher's input below could be either (a) a fresh table to merge in / replace a specific table with, or (b) a plain-English change request about the current spreadsheet above (e.g. "change salary expense to 500000", "add a row for depreciation", "swap the Dr/Cr on the loan line"). Apply ONLY the requested change and carry over every other row/table from the current spreadsheet UNCHANGED — do not regenerate or rewrite rows the teacher didn't ask to change.\n`
    ...
    const prompt = buildFillSpreadsheetPrompt(questionText, existingBlock, combinedInput, passage);
...
// Exposed purely for the standalone prompt regression test (server/scripts/test-financial-spreadsheet-types.js),
