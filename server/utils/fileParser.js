...
ACCOUNTING KNOWLEDGE TO APPLY TO FINANCIAL-SPREADSHEET QUESTIONS - get the classification, ordering and layout right, matching how a real marking guide presents each statement type:
1) STATEMENT OF FINANCIAL POSITION: Non-Current Assets -> "Total Non-Current Assets" -> Current Assets (Inventories, Receivables, Prepayments, Cash and Cash Equivalents) -> "Total Current Assets" -> "Total Assets"; then Equity -> "Total Equity" -> Non-Current Liabilities -> "Total Non-Current Liabilities" -> Current Liabilities -> "Total Current Liabilities" -> "Total Equity and Liabilities" (must equal Total Assets).
2) INCOME STATEMENT: Revenue -> Cost of Sales -> "Gross Profit" -> Other Income -> Operating Expenses (each expense in column 2, one "Total Operating Expenses" in column 3) -> "Operating Profit" -> Finance costs -> "Profit Before Tax" -> Tax -> "Profit for the Year".
3) STATEMENT OF CASH FLOWS (IAS 7, indirect method): Operating Activities (Profit Before Tax + non-cash items +/- working-capital movements, less interest and tax paid) -> Investing Activities -> Financing Activities -> "Net Increase/(Decrease) in Cash" -> opening cash -> "Cash at End" (reconciles to the SOFP's cash figure). Derived figures (depreciation charge, tax paid, disposal gain/loss, interest paid) that aren't given directly should be shown as separate small "WK1", "WK2", ... tables alongside the main statement.
4) CASH BOOK / ADJUSTED CASH BOOK: ONE table, headers ["Details (Dr)","Amount","Details (Cr)","Amount"] - corrects the business's own cash/bank balance for bank charges, standing orders, dishonoured cheques, direct deposits, and casting errors (NOT timing differences - those go in the Bank Reconciliation Statement). Ends with a balancing "Bal c/d" figure and the same total under both amount columns. If the source instead shows a single running-balance cash book (one list, Balance recalculated after every row, not two side-by-side Dr/Cr columns), use headers ["Date","Particulars","Receipts","Payments","Balance"] instead.
4b) SALES JOURNAL / PURCHASES JOURNAL / RETURNS JOURNALS (Day Books): headers ["Date","Customer","Ref","Amount"] (Sales/Returns Inwards) or ["Date","Supplier","Ref","Amount"] (Purchases/Returns Outwards) - ONE row per credit transaction, ending in a "Monthly total" row. Single amount column only (not Debit/Credit) - list credit transactions only, never cash/cheque items or capital purchases (those belong in the Cash Book, not here). Keep as its own separate table even when a Cash Book is also required in the same question.
5) BANK RECONCILIATION STATEMENT: 3-column layout like the statements above (["Item","FRW ...","FRW ..."]), starting from the adjusted cash book balance, "Add:" row then unpresented cheques, "Less:" row then uncredited deposits, ending in the balance as per bank statement.
6) RATIO ANALYSIS: ONE comparative table, headers ["Ratio","<Year 1>","<Year 2>",...], one row per ratio, each value a formatted string with its unit ("X:1", "X%", "X days", "X times") - not a bare number.
7) Trial Balance: headers ["Item","Debit","Credit"], Debit total = Credit total. Ledger/T-accounts: one table per account, headers ["Date","Particulars","Debit","Credit"], preserving exactly which side each entry is on. Journal entries: same headers, debited account's amount under Debit, credited account's amount under Credit (prefix "To ...").
8) PUBLIC SECTOR / IPSAS (ministries): "Statement of Financial Performance" (Revenue lines -> "Total revenue"; Expenditure lines -> "Surplus/(Deficit) for the year") and "Statement of Financial Position" (same structure as a company SOFP but "Accumulated Surplus/(Deficit)" instead of Retained Earnings).
9) OTHER CPA/FINANCE SCHEDULES: Control accounts (Receivables/Payables) - ledger shape. Suspense account - ledger shape, plus a "Statement of Corrected Net Profit" (Item/Amount). Manufacturing account - statement shape, ending "Cost of Production". Statement of Affairs (incomplete records or non-profit accumulated fund) - same shape as SOFP. Partnership Appropriation Account - statement shape; Partners' Capital/Current Accounts - COLUMNAR by partner, headers ["Item","<Partner A>","<Partner B>",...]. Receipts and Payments Account - same 4-column shape as Cash Book; Income and Expenditure Account - same shape as Income Statement, ending "Surplus/(Deficit)". Consolidated SOFP/Income Statement - same shape as single-entity versions plus a "Non-Controlling Interest" equity line and a goodwill working. PPE note - COLUMNAR by asset class, headers ["Item","<Asset 1>","<Asset 2>",...,"Total"]; Disposal Account - ledger shape. Budgets - COLUMNAR by period, headers ["Item","<Period 1>","<Period 2>",...,"Total"]. Standard costing variances - headers ["Variance","Amount","Favourable/Adverse"] plus a profit-reconciliation statement. Cost Sheet - statement shape ending "Sales"; Process Account - ledger shape with BOTH Units and Amount columns per side; Contract Account - ledger shape plus notional-profit/retention workings. Tax computation - statement shape ending "Tax Payable"; Capital Allowances Schedule - COLUMNAR by asset class like the PPE note. Investment appraisal (NPV/IRR/Payback) - headers ["Year","Cash Flow","Discount Factor","Present Value"]. Loan/Bond Amortization Schedule - headers ["Period","Opening Balance","Interest","Repayment","Closing Balance"].
...
- EVERY VALUE CELL MUST BE A PLAIN FINAL NUMBER, NEVER AN ARITHMETIC EXPRESSION: do the addition/subtraction yourself and write only the result (e.g. 225000, not "20000 + 280000 - 25000"). EXCEPTION: in a Ratio Analysis table only, a value cell is a formatted string including its unit (e.g. "26.8%", "2.5:1", "49 days", "9 times").
...
4) STATEMENT OF CASH FLOWS (IAS 7, indirect method unless the direct method is explicitly asked for) - three sections in order: Operating Activities (start from Profit Before Tax, add back non-cash items like Depreciation/Interest/loss on disposal, deduct gains on disposal, adjust for working-capital movements, giving "Cash generated from operations", then less interest paid and tax paid) -> Investing Activities (purchase/sale of non-current assets, intangibles, investments; interest/dividends received) -> Financing Activities (shares/loans issued, loans repaid, dividends paid) -> "Net Increase/(Decrease) in Cash and Cash Equivalents" -> Cash at start -> "Cash at End" (must reconcile to Cash and Cash Equivalents in the SOFP; a negative/overdrawn balance is shown in brackets). Figures rarely given directly (depreciation charge, tax paid, disposal proceeds/gain-or-loss, interest paid) must be derived by reconstructing the relevant account from opening/closing balances - add each derivation as its OWN small table ("WK1: ...", "WK2: ...", etc.) in the same tables array alongside the main statement.

4b) CASH BOOK / ADJUSTED CASH BOOK - corrects the business's own cash/bank balance for items it hasn't recorded yet but the bank has (bank charges, standing orders, dishonoured cheques, interest, direct deposits, and correction of the business's own casting errors). NEVER include a timing difference here (unpresented cheques / uncredited deposits belong in the Bank Reconciliation Statement instead). Start from the existing cash book balance, add items that increase it, deduct items that decrease it, to reach the adjusted balance.

4c) BANK RECONCILIATION STATEMENT - reconciles the ADJUSTED cash book balance to the bank statement balance for TIMING differences only: starting from the adjusted cash book balance, add unpresented/outstanding cheques, less uncredited/outstanding deposits, to reach the balance as per bank statement (or the reverse direction, per the question's own wording).

4d) SALES JOURNAL / PURCHASES JOURNAL / RETURNS JOURNALS (Day Books) - a dated LISTING of credit transactions only (no cash/cheque items, no capital purchases), one row per invoice, ONE amount column, ending in a "Monthly total" row. Never a Debit/Credit table, and never merged into the Cash Book even when required by the same question.
...
7) RATIO ANALYSIS - when the question asks to compute/interpret ratios (current ratio, acid-test ratio, gross profit margin, ROCE, payables/receivables/inventory period, debt-to-equity, times interest cover, asset turnover, etc.) across one or more years, produce ONE comparative table: one row per ratio, one column per year. Each value cell is a formatted string in the ratio's natural unit ("X:1", "X%", "X days", "X times") - never a bare number. Any interpretation asked for is prose, not part of the table.

8) PUBLIC SECTOR / IPSAS STATEMENTS (ministries, public entities) - "Statement of Financial Performance" is the public-sector Income Statement equivalent (Revenue: transfers from exchequer, fees and fines, transfers from other ministries, other non-exchange revenues -> "Total revenue"; Expenditure: salaries and wages, office consumables, transfers to other ministries, finance cost, depreciation, other expenses -> "Surplus/(Deficit) for the year"). "Statement of Financial Position" follows the same structure as a company SOFP but replaces Retained Earnings with "Accumulated Surplus/(Deficit)" and may include a "Liability for long-term employee benefits" line; it must still balance (Total Assets = Total Capital and Liabilities).

9) OTHER CPA/FINANCE SCHEDULES - build exactly the one asked for, in its conventional layout: Control accounts (Receivables/Payables) - ledger shape (["Date","Particulars","Debit","Credit"]). Suspense account - ledger shape, plus a "Statement of Corrected Net Profit" (Item/Amount). Manufacturing account - statement shape ending "Cost of Production". Statement of Affairs (incomplete records, or a non-profit's accumulated fund) - same shape as an SOFP. Partnership Appropriation Account - statement shape; Partners' Capital/Current Accounts - COLUMNAR by partner, headers ["Item","<Partner A>","<Partner B>",...]; Realization Account (on dissolution) - ledger shape. Receipts and Payments Account - same 4-column shape as the Cash Book; Income and Expenditure Account - same shape as the Income Statement, ending "Surplus/(Deficit)". Consolidated SOFP/Income Statement - same shape as the single-entity versions plus a "Non-Controlling Interest" equity line and a goodwill-on-acquisition working. PPE note - COLUMNAR by asset class, headers ["Item","<Asset 1>","<Asset 2>",...,"Total"]; Disposal Account - ledger shape. Budgets - COLUMNAR by period, headers ["Item","<Period 1>","<Period 2>",...,"Total"]. Standard costing variances - headers ["Variance","Amount","Favourable/Adverse"] plus a profit-reconciliation statement. Cost Sheet - statement shape ending "Sales"; Process Account - ledger shape with BOTH Units and Amount columns per side; Contract Account - ledger shape plus notional-profit/retention workings. Tax computation - statement shape ending "Tax Payable"; Capital Allowances Schedule - COLUMNAR by asset class like the PPE note. Investment appraisal (NPV/IRR/Payback) - headers ["Year","Cash Flow","Discount Factor","Present Value"]. Loan/Bond Amortization Schedule - headers ["Period","Opening Balance","Interest","Repayment","Closing Balance"].

FORMAT BY DOCUMENT TYPE:
  * Statement of Financial Position, Income Statement, Statement of Changes in Equity, Statement of Cash Flows, Statement of Financial Performance (IPSAS), Bank Reconciliation Statement, Manufacturing Account, Statement of Affairs, Partnership Appropriation Account, Income and Expenditure Account, Cost Sheet, Tax computation: headers like ["Item","FRW '000'","FRW '000'"] - column 1 is the item/section heading (section headers, or "Add:"/"Less:" in a bank reconciliation, get their own row with no figures), column 2 holds sub-amounts/workings for a line needing several components added together (leave "" when not needed), column 3 holds the figure that carries into the statement's running total. Do NOT use "Debit"/"Credit" headers for these, and do NOT turn section names into column headers. For a Cash Flow Statement, put derived workings (WK1, WK2, ...) in their own separate tables rather than inside the main statement's rows.
  ...
  * Ledger accounts / T-accounts, Control accounts, Disposal Account, Suspense Account, Realization Account: one table per account, headers ["Date","Particulars","Debit","Credit"], preserving exactly which side each entry is on as the source shows it; whichever side isn't used must be "" not 0.
  * Journal entries: headers ["Date","Particulars","Debit","Credit"], one row per account touched by each entry.
  * Cash Book / Adjusted Cash Book, Receipts and Payments Account: ONE table, headers ["Details (Dr)","Amount","Details (Cr)","Amount"] (use "(Dr)"/"(Cr)", not the words "Debit"/"Credit") - columns 1-2 are the debit/receipts side (opening balance plus items that increase it), columns 3-4 are the credit/payments side (items that decrease it, ending with the balancing "Bal c/d" on whichever side is smaller); leave the other side's cells "" (never 0) on a row where it has no entry. The final row's total must be identical under both amount columns. If the source instead shows a single running-balance cash book (one list, Balance recalculated after every row, not two side-by-side columns), use headers ["Date","Particulars","Receipts","Payments","Balance"] instead.
  * Sales Journal / Purchases Journal / Returns Journals (Day Books): headers ["Date","Customer","Ref","Amount"] (Sales/Returns Inwards) or ["Date","Supplier","Ref","Amount"] (Purchases/Returns Outwards) - one row per credit transaction, ending in a "Monthly total" row; single amount column, never Debit/Credit.
  * Bank Reconciliation Statement: same 3-column layout as the statements above - start row is the adjusted cash book (or bank statement) balance, "Add:" as its own row followed by increasing items, "Less:" as its own row followed by decreasing items (write deductions as negative, e.g. "(1,800)"), ending in the final reconciled balance.
  * Ratio Analysis: headers ["Ratio","<Year 1>","<Year 2>",...] (add "Formula" only if the source shows it) - one row per ratio, one column per year, each value cell a formatted string with its unit (e.g. "3.14:1", "28.8%", "32 days", "15.36 times").
  * Partners' Capital/Current Accounts, PPE note, Capital Allowances Schedule, Budgets: COLUMNAR - headers ["Item","<column 1>","<column 2>",...,"Total"], one column per partner/asset-class/period rather than a 2-column Dr/Cr layout.
  * Process Account: ledger shape but headers ["Particulars","Units","Amount","Particulars","Units","Amount"] (quantity AND value on each side).
  * Investment appraisal (NPV/IRR/Payback): headers ["Year","Cash Flow","Discount Factor","Present Value"]. Loan/Bond Amortization Schedule: headers ["Period","Opening Balance","Interest","Repayment","Closing Balance"].
  * Anything else (schedule, workings): whichever layout is standard for that output; only use Debit/Credit columns when double-entry actually applies.

Before finalizing, recompute every subtotal independently and sanity-check: does the Statement of Financial Position balance (Total Assets = Total Equity and Liabilities)? Does the Trial Balance balance? Does the Cash Book's Dr total equal its Cr total? Does the Bank Reconciliation Statement's result equal the figure it's meant to reconcile to? Does the Cash Flow Statement's closing cash equal the SOFP's cash figure? Fix any mismatch rather than returning inconsistent numbers.
